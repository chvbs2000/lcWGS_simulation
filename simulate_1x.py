...
class simulator:
    def __init__(self):
        ...
        """
        #model directory
        self.model_dir = "/gpfs/home/kchen/bayes/lowpass_GL_bayes_inference/gmm_1x_AFR-EUR"
        self.name="noOverorginalformula"
...
#         self.gmm_homoalt.covariances_ = self.covar_homoalt
        ...
        self.means_homoref = np.load(self.model_dir + "/" + "gmm_{}_1x_homozygous_reference_means.npy".format(str(self.name)))
        self.covar_homoref = np.load(self.model_dir + "/" + "gmm_{}_1x_homozygous_reference_covariances.npy".format(str(self.name)))
        self.gmm_homoref = mixture.GaussianMixture(n_components = len(self.means_homoref), covariance_type='full')
        self.gmm_homoref.precisions_cholesky_ = np.linalg.cholesky(np.linalg.inv(self.covar_homoref))
        self.gmm_homoref.weights_ = np.load(self.model_dir + "/" + "gmm_{}_1x_homozygous_reference_weights.npy".format(str(self.name)))
        self.gmm_homoref.means_ = self.means_homoref
        self.gmm_homoref.covariances_ = self.covar_homoref

        # heterozygous model
        self.means_het = np.load(self.model_dir + "/" + "gmm_{}_1x_heterozygous_means.npy".format(str(self.name)))
        self.covar_het = np.load(self.model_dir + "/" + "gmm_{}_1x_heterozygous_covariances.npy".format(str(self.name)))
        self.gmm_het = mixture.GaussianMixture(n_components = len(self.means_het), covariance_type='full')
        self.gmm_het.precisions_cholesky_ = np.linalg.cholesky(np.linalg.inv(self.covar_het))
        self.gmm_het.weights_ = np.load(self.model_dir + "/" + "gmm_{}_1x_heterozygous_weights.npy".format(str(self.name)))
        self.gmm_het.means_ = self.means_het
        self.gmm_het.covariances_ = self.covar_het
        
        # homozygous alternative model
        self.means_homoalt = np.load(self.model_dir + "/" + "gmm_{}_1x_homozygous_alternative_means.npy".format(str(self.name)))
        self.covar_homoalt = np.load(self.model_dir + "/" + "gmm_{}_1x_homozygous_alternative_covariances.npy".format(str(self.name)))
        self.gmm_homoalt = mixture.GaussianMixture(n_components = len(self.means_homoalt), covariance_type='full')
        self.gmm_homoalt.precisions_cholesky_ = np.linalg.cholesky(np.linalg.inv(self.covar_homoalt))
        self.gmm_homoalt.weights_ = np.load(self.model_dir + "/" + "gmm_{}_1x_homozygous_alternative_weights.npy".format(str(self.name)))
        self.gmm_homoalt.means_ = self.means_homoalt
        self.gmm_homoalt.covariances_ = self.covar_homoalt
...
if __name__ == "__main__":
    ...
    #parser.add_argument("--gmm_name", type=str, default="No_name", help=" GMM model name (default: No_name)") 
    ...
    result = subprocess.run(extract_pos_cmd, shell=True, capture_output=True, text=True)
